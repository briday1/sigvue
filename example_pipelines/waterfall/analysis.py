...
class WaterfallAnalysis(
    Analysis[SigMFWindow, WaterfallSettings, WaterfallProducts]
):
    """Compute a waterfall and average spectrum for a SigMF window."""
    ...
    def process(
        ...
        if settings is None:
            raise RuntimeError(
                "Waterfall analysis requires configured settings"
            )
        ...
        hop = max(
            1,
            round(
                fft_size * (1 - settings.overlap_percent / 100)
            ),
        )
        starts = np.arange(
            0,
        ...
        return WaterfallProducts(
            ...
            frequency,
            (
                (data.start_sample + time_edges)
                / data.recording.sample_rate
                * 1e3
            ),
            data.buffer_nbytes,
        )
