...
def analysis_object(process, configure=None):
    return Analysis(process, configure)
...
class WorkspaceContractTests(unittest.TestCase):
    ...
    def test_public_source_and_delivery_are_explicit_framework_objects(self):
        ...
        with self.assertRaisesRegex(TypeError, r"requires a prepare callback"):
            IncompleteDelivery()

        class IncompleteSource(Source[list[int]]):
            pass

        with self.assertRaisesRegex(TypeError, r"requires discover/open callbacks"):
            IncompleteSource()

        class MissingOpen(Source[list[int]]):
            def discover(self):
                return ()

        with self.assertRaisesRegex(TypeError, r"both overridden methods"):
            MissingOpen()

        with self.assertRaisesRegex(TypeError, r"requires both discover and open"):
            Source(lambda: ())

    def test_framework_lifecycle_objects_accept_callbacks_directly(self):
        resource = DataResource("one", "One", source=5)
        source = Source(lambda: (resource,), lambda selected: selected.source)
        delivery = Delivery(lambda value, ui: value * 2)
        analysis = Analysis(lambda value, settings: value + 1)
        ...
        workspace = Workspace._from_runtime_components(
            identifier="framework-objects",
            name="Framework objects",
            description="Callback-backed lifecycle objects",
            source=source,
            delivery=delivery,
            analysis=analysis,
        ...
        self.assertEqual("11", workspace.open_item("one").page.views[0].callback({}))

    def test_source_and_delivery_subclasses_compose_explicitly(self):
        resource = DataResource("one", "One", source=5)
    ...
    def test_workspace_rejects_incomplete_contracts_with_actionable_errors(self):
        ...
        with self.assertRaisesRegex(TypeError, r"requires a process callback"):
            MissingProcess()
        with self.assertRaisesRegex(TypeError, r"requires a present callback"):
            MissingPresent()
    ...
    def test_select_mapping_keeps_internal_values_and_exposes_display_labels(self):
        ui = AnalysisContext({"detector": "outer_scintillator"})
        selected = ui.select(
            "detector",
            default="large_detector",
            options={
                "large_detector": "Large detector",
                "outer_scintillator": "Outer scintillator",
            },
        )

        self.assertEqual("outer_scintillator", selected)
        self.assertEqual(
            ("large_detector", "outer_scintillator"),
            ui.controls[0].options,
        )
        self.assertEqual(
            ("Large detector", "Outer scintillator"),
            ui.controls[0].option_labels,
        )
