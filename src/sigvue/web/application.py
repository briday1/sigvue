...
function startFrameworkWindowed(config,refresh){
  ...
  const heatColor=value=>{const stops=[[48,18,59],[50,104,172],[39,186,142],[237,225,65],[180,4,38]],scaled=Math.min(1,Math.max(0,value))*(stops.length-1),index=Math.min(stops.length-2,Math.floor(scaled)),mix=scaled-index;return stops[index].map((channel,offset)=>Math.round(channel+(stops[index+1][offset]-channel)*mix))};
  const prepareHeatmap=()=>{const rows=(config.overview_heatmap||[]).map(row=>row.map(Number));if(!rows.length||!rows[0]?.length)return null;if(heatmapCanvas)return heatmapCanvas;const rowCount=rows.length,columnCount=rows[0].length,values=rows.flat().sort((left,right)=>left-right),low=values[Math.floor((values.length-1)*.02)],high=values[Math.ceil((values.length-1)*.995)],span=high-low||1;heatmapCanvas=document.createElement('canvas');heatmapCanvas.width=columnCount;heatmapCanvas.height=rowCount;const context=heatmapCanvas.getContext('2d'),image=context.createImageData(columnCount,rowCount);rows.forEach((row,rowIndex)=>row.forEach((value,columnIndex)=>{const color=heatColor((value-low)/span),offset=((rowCount-1-rowIndex)*columnCount+columnIndex)*4;image.data[offset]=color[0];image.data[offset+1]=color[1];image.data[offset+2]=color[2];image.data[offset+3]=220}));context.putImageData(image,0,0);return heatmapCanvas};
