...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } [hidden] { display:none!important } body { margin:0; font:15px/1.5 system-ui,-apple-system,sans-serif; color:var(--ink); background:#fbfcfc }
    ...
    .notification-center { position:relative } .notification-center>summary { display:flex; min-height:30px; align-items:center; gap:6px; padding:4px 8px; border:1px solid #b9d0d54d; border-radius:6px; color:#e7f1f3; background:#193741; font:600 12px/20px system-ui,sans-serif; cursor:pointer; list-style:none } .notification-center>summary::-webkit-details-marker { display:none } .notification-center[open]>summary { border-color:#8ed0d7 } .notification-badge { display:grid; min-width:18px; height:18px; padding:0 5px; place-items:center; border-radius:999px; color:#102f3a; background:#8ed0d7; font-size:10px } .notification-popover { position:absolute; z-index:80; top:38px; right:0; width:min(440px,calc(100vw - 24px)); max-height:min(560px,calc(100vh - 70px)); overflow:auto; border:1px solid var(--line); border-radius:9px; color:var(--ink); background:#fbfcfc; box-shadow:0 14px 32px #071b2240 } html[data-theme="dark"] .notification-popover { background:#193741 } .notification-head { display:flex; align-items:center; justify-content:space-between; padding:11px 13px; border-bottom:1px solid var(--line) } .notification-head strong { font-size:13px } .notification-empty { margin:0; padding:18px; color:var(--muted); font-size:12px; text-align:center } .notification-item { padding:12px 13px; border-bottom:1px solid var(--line) } .notification-item:last-child { border-bottom:0 } .notification-title { display:flex; align-items:start; gap:8px } .notification-title strong { flex:1; color:var(--ink); font-size:13px } .notification-status { flex:none; font-size:11px; font-weight:700; text-transform:uppercase } .notification-status.ready { color:#16803c } .notification-status.error { color:#b42318 } .notification-dismiss { flex:none; padding:0 4px; border:0; color:var(--muted); background:transparent; font-size:18px; line-height:18px; cursor:pointer } .notification-summary { margin:4px 0 0; color:var(--muted); font-size:12px } .notification-files { display:grid; gap:6px; margin-top:9px }
...
function bindPlotSelection(plot){if(plot.dataset.plotSelectionBound)return;plot.dataset.plotSelectionBound='true';plot.on('plotly_selected',event=>{const ranges=selectedPlotRanges(plot,event);if(Object.keys(ranges).length)plotSelections.set(plot.dataset.plotView,ranges)});plot.on('plotly_relayouting',event=>constrainPlotDuringPan(plot,event));const clear=()=>plotSelections.delete(plot.dataset.plotView);plot.on('plotly_deselect',clear);plot.on('plotly_doubleclick',()=>{clear();resetPlotAxes(plot);return false})}
...
const clientRuntimeRows='<div><dt>Plotly render</dt><dd data-client-stat="plotly-runtime">—</dd></div><div><dt>Browser render total</dt><dd data-client-stat="browser-runtime">—</dd></div>';
...
async function openItem(wid,wname,iid,navigate=true,controlValues={},preservePlayback=false){
  ...
  try{const request=async values=>api(`/workspaces/${encodeURIComponent(wid)}/items/${encodeURIComponent(iid)}?${new URLSearchParams(values)}`),windowValues=()=>windowEnd==null?{}:{__window_start_seconds:windowStart,__window_end_seconds:windowEnd},segmentValues=()=>segmentId==null?{}:{__segment_id:segmentId};let data=await request({...controlValues,...windowValues(),...segmentValues(),__theme:resolvedTheme(),__playback_time_seconds:playbackPosition});let p=data.page,requestGeneration=0;const isPlayback=['seek','live'].includes(p.playback.mode),isWindowed=p.playback.mode==='windowed',isSegmented=p.playback.mode==='segmented';annotations=p.annotation?.entries||[];
    ...
    const refresh=async(includeStatic=false,commitRequestedTheme=false)=>{const browserStarted=performance.now(),generation=++requestGeneration,result=await request({...selected(),__playback_time_seconds:playbackPosition,__include_static_views:includeStatic});if(generation!==requestGeneration)return false;data=result;Object.assign(playbackConfig,result.page.playback);p=result.page;p.playback=playbackConfig;annotationTimelineColorControl=p.annotation?.timeline_color_control||null;if(Array.isArray(p.annotation?.entries))annotations=p.annotation.entries;if(playbackFollowLive)playbackPosition=playbackConfig.duration_seconds;if(commitRequestedTheme)await preloadMatplotlibViews(p.rendered_views);const render=async()=>{if(commitRequestedTheme)applyTheme();updateStatistics(p.statistics,p.runtime_statistics);await updatePlotlyViews(p.rendered_views);await updateMatplotlibViews(p.rendered_views);updateGenericViews(p.rendered_views);setClientRuntime('browser-runtime',performance.now()-browserStarted)};if(commitRequestedTheme)await commitTheme(render);else await render();return true};activeThemeRefresh=async()=>{if(isPlayback)clearInterval(playbackTimer);const applied=await refresh(true,true);if(isPlayback&&applied)startFrameworkPlayback(p.playback,refresh);else if(isWindowed&&applied)startFrameworkWindowed(p.playback,refresh);else if(isSegmented&&applied)startFrameworkSegmented(p.playback,refresh)};
