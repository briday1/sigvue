...
function syncViewportControls(view,viewport){let changed=false;for(const [axis,names] of Object.entries(view?.viewport_controls||{})){const range=viewport?.[axis];if(!Array.isArray(range)||range.length<2||!Array.isArray(names)||names.length<2)continue;const edges=range.map(Number).sort((a,b)=>a-b);for(const [index,name] of names.entries()){const input=document.querySelector(`[data-control="${CSS.escape(String(name))}"]`);if(input&&Number.isFinite(edges[index])){input.value=Number(edges[index].toPrecision(12));changed=true}}}return changed}
function bindPlotSelection(plot,view,onViewportChanged){if(plot.dataset.plotSelectionBound)return;plot.dataset.plotSelectionBound='true';plot.on('plotly_selected',event=>{const ranges=selectedPlotRanges(plot,event);if(Object.keys(ranges).length)plotSelections.set(plot.dataset.plotView,ranges)});plot.on('plotly_relayouting',event=>constrainPlotDuringPan(plot,event));plot.on('plotly_relayout',event=>{if(plot._sigvueUpdating||plot._sigvueResetting)return;if(requestsPlotReset(event)){plotSelections.delete(plot.dataset.plotView);plot._sigvueViewport={};resetPlotAxes(plot,onViewportChanged);return}const viewport=capturePlotViewport(plot,event);if(Object.keys(viewport).length){syncViewportControls(view,viewport);onViewportChanged?.()}});const clear=()=>plotSelections.delete(plot.dataset.plotView);plot.on('plotly_deselect',clear);plot.on('plotly_doubleclick',()=>{clear();plot._sigvueViewport={};resetPlotAxes(plot,onViewportChanged);return false})}
...
function bindLimitsPickers(onCommit){document.querySelectorAll('[data-limits-picker]').forEach(picker=>{const hidden=picker.querySelector('[data-control]'),lowerNumber=picker.querySelector('[data-limit-number="lower"]'),upperNumber=picker.querySelector('[data-limit-number="upper"]'),lowerRange=picker.querySelector('[data-limit-range="lower"]'),upperRange=picker.querySelector('[data-limit-range="upper"]'),track=picker.querySelector('.limits-slider-track'),minimum=Number(lowerNumber.min),maximum=Number(lowerNumber.max),step=Number(lowerNumber.step)||1;const sync=(changed,value,commit)=>{let lower=Number(lowerNumber.value),upper=Number(upperNumber.value),next=Number(value);if(!Number.isFinite(next))next=changed==='lower'?lower:upper;if(changed==='lower')lower=Math.min(upper-step,Math.max(minimum,next));else upper=Math.max(lower+step,Math.min(maximum,next));lowerNumber.value=lower;upperNumber.value=upper;lowerRange.value=lower;upperRange.value=upper;hidden.value=`${lower},${upper}`;const span=Math.max(Number.EPSILON,maximum-minimum);track.style.setProperty('--lower',`${100*(lower-minimum)/span}%`);track.style.setProperty('--upper',`${100*(upper-minimum)/span}%`);if(commit){if(onCommit)void onCommit();else hidden.dispatchEvent(new Event('change',{bubbles:true}))}};lowerNumber.onchange=event=>sync('lower',event.target.value,true);upperNumber.onchange=event=>sync('upper',event.target.value,true);lowerRange.oninput=event=>sync('lower',event.target.value,false);upperRange.oninput=event=>sync('upper',event.target.value,false);lowerRange.onchange=event=>sync('lower',event.target.value,true);upperRange.onchange=event=>sync('upper',event.target.value,true);lowerNumber.onkeydown=upperNumber.onkeydown=event=>{if(event.key==='Enter')event.currentTarget.blur()};sync('lower',lowerNumber.value,false)})}
