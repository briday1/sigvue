...
function limitsPickerHtml(control,values){const [lower,upper]=limitsValue(control,values),minimum=Number(control.minimum),maximum=Number(control.maximum),step=Number(control.step)||1,reset=control.resettable?`<button class="limits-reset" type="button" data-limits-reset aria-label="Reset ${esc(control.label||controlLabel(control.name))}">Reset</button>`:'';return `<div class="limits-picker" data-limits-picker data-commit-mode="${esc(control.commit_mode||'change')}" data-default-lower="${control.default[0]}" data-default-upper="${control.default[1]}"><div class="limits-picker-head"><span class="limits-picker-name">${esc(control.label||controlLabel(control.name))}</span><input type="number" data-limit-number="lower" value="${lower}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Lower limit"><span class="limits-separator">to</span><input type="number" data-limit-number="upper" value="${upper}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Upper limit">${reset}</div><div class="limits-slider"><div class="limits-slider-track"></div><input type="range" data-limit-range="lower" value="${lower}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Lower limit slider"><input type="range" data-limit-range="upper" value="${upper}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Upper limit slider"></div><input type="hidden" data-control="${esc(control.name)}" value="${lower},${upper}"></div>`}
