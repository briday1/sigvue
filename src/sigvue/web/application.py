...
function startFrameworkSegmented(config,refresh){
  const root=document.querySelector('#segmented-bar'),track=root?.querySelector('#segmented-track'),previous=root?.querySelector('#segment-previous'),next=root?.querySelector('#segment-next'),toggle=root?.querySelector('#segment-toggle'),rateInput=root?.querySelector('#segment-rate'),stepInput=root?.querySelector('#segment-step'),counter=root?.querySelector('#segment-count'),time=root?.querySelector('#segment-time');if(!root||!track)return;const lifecycle=++segmentedPlaybackGeneration;clearTimeout(playbackTimer);playbackTimer=null;let updating=false,queuedDirection=0,autoPlaying=false;
  ...
  const render=()=>{const segments=available();if(!segments.length)return;if(!segments.some(segment=>segment.identifier===segmentId))segmentId=config.selected_segment_id||segments[0].identifier;const index=selectedIndex(),selected=segments[index],duration=Number(config.duration_seconds)||0,unit=timelineSpec(config).label,disabled=segments.length<2;track.innerHTML=`<div class="annotation-markers" data-annotation-markers></div>`+segments.map(segment=>{const percent=duration?Math.min(100,Math.max(0,Number(segment.start_seconds)/duration*100)):0,label=segment.label||segment.identifier,title=`${label} · ${fixedSegmentTime(segment.start_seconds,config)} ${unit} · ${fixedSegmentTime(segment.duration_seconds,config)} ${unit}`;return `<button class="segment-marker ${segment.identifier===segmentId?'active':''}" type="button" style="left:${percent}%" data-segment-id="${esc(segment.identifier)}" aria-label="${esc(title)}" title="${esc(title)}"></button>`}).join('');counter.textContent=`${index+1} / ${segments.length}`;time.textContent=formatSegmentRange(selected,duration,config);previous.disabled=next.disabled=toggle.disabled=disabled;stepInput.max=String(Math.max(1,segments.length-1));stepInput.value=String(playbackStep());if(autoPlaying&&disabled)stopAuto();track.querySelectorAll('[data-segment-id]').forEach(marker=>marker.onclick=()=>select(marker.dataset.segmentId));renderAnnotationMarkers(config)};
  const select=async identifier=>{if(updating||identifier===segmentId)return false;segmentId=identifier;render();updating=true;try{await refresh();return true}finally{updating=false;render();if(queuedDirection){const direction=queuedDirection;queuedDirection=0;void advance(direction)}}};
