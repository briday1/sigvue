...
function startFrameworkSegmented(config,refresh){
  ...
  const nearestSegment=clientX=>{const segments=available();if(!segments.length)return null;const bounds=track.getBoundingClientRect(),usableWidth=Math.max(1,bounds.width-trackInset*2),fraction=Math.min(1,Math.max(0,(clientX-bounds.left-trackInset)/usableWidth));return segments.reduce((nearest,segment,index)=>Math.abs(segmentFraction(segment,index,segments)-fraction)<Math.abs(segmentFraction(nearest.segment,nearest.index,segments)-fraction)?{segment,index}:nearest,{segment:segments[0],index:0}).segment};
