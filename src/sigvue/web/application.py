...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } [hidden] { display:none!important } body { margin:0; font:15px/1.5 system-ui,-apple-system,sans-serif; color:var(--ink); background:#fbfcfc }
    ...
    .batch-controls { display:flex; width:132px; align-items:center; justify-content:flex-end; gap:6px } .batch-menu,.batch-browse-menu { position:relative; z-index:5 } .batch-menu summary { position:relative; display:grid; width:34px; height:34px; padding:0; place-items:center; border:1px solid var(--line); border-radius:50%; color:var(--accent); background:var(--wash); cursor:pointer; list-style:none } .batch-menu summary::-webkit-details-marker,.batch-folder::-webkit-details-marker { display:none } .batch-menu[open] summary,.batch-browse-menu[open]>.batch-folder { border-color:var(--accent); box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 14%,transparent) } .batch-folder { display:flex; width:92px; height:34px; align-items:center; justify-content:center; gap:6px; border:1px solid var(--line); border-radius:7px; color:var(--accent); background:var(--wash); font:650 11px system-ui,sans-serif; cursor:pointer; text-decoration:none; list-style:none } .batch-folder:hover { border-color:var(--accent); background:color-mix(in srgb,var(--accent) 8%,var(--wash)); box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 10%,transparent) } .batch-folder[aria-disabled="true"] { color:var(--muted); opacity:.38; pointer-events:none } .batch-folder svg { width:17px; height:17px; flex:none; fill:none; stroke:currentColor; stroke-width:1.7; stroke-linecap:round; stroke-linejoin:round } .batch-folder-label { line-height:1 } .batch-browse-caret { margin-left:-2px; color:var(--muted); font-size:9px } .batch-play { margin-left:2px; font-size:15px; line-height:1 } .batch-menu-popover,.batch-browse-popover { position:absolute; z-index:30; top:40px; right:0; width:320px; padding:8px; border:1px solid var(--line); border-radius:8px; background:#fbfcfc; box-shadow:0 12px 28px #102f3a30 } html[data-theme="dark"] .batch-menu-popover,html[data-theme="dark"] .batch-browse-popover { background:#193741 } .batch-browse-popover { width:280px } .batch-browse-choice { display:flex; width:100%; min-height:38px; align-items:center; gap:9px; padding:8px 9px; border-radius:6px; color:var(--ink); font-size:12px; text-decoration:none } .batch-browse-choice:hover { color:var(--accent); background:var(--wash) } .batch-browse-choice svg { width:17px; height:17px; flex:none; fill:none; stroke:currentColor; stroke-width:1.7; stroke-linecap:round; stroke-linejoin:round } .batch-browse-choice+.batch-browse-choice { border-top:1px solid var(--line); border-radius:0 } .batch-action-row+.batch-action-row { border-top:1px solid var(--line) } .batch-action { display:grid; width:100%; grid-template-columns:1fr auto; gap:8px; padding:9px; border:0; border-radius:6px; color:var(--ink); background:transparent; text-align:left; cursor:pointer } .batch-action:hover { background:var(--wash) } .batch-artifacts { display:grid; gap:5px; padding:0 9px 8px; font-size:12px } .batch-artifact { display:flex; min-width:0; align-items:center; gap:6px } .batch-path { min-width:0; flex:1; overflow:hidden; color:var(--muted); text-overflow:ellipsis; white-space:nowrap } .batch-open { color:var(--accent) } .copy-path { flex:none; padding:3px 7px; border:1px solid var(--line); border-radius:5px; color:var(--ink); background:var(--wash); font:600 11px system-ui,sans-serif; cursor:pointer } .copy-path:hover { border-color:var(--accent); color:var(--accent) } .batch-state { color:var(--muted); font-size:12px } .batch-state.running,.batch-state.pending { color:#b7791f } .batch-state.ready { color:#16803c } .batch-state.error { color:#b42318 } .item-browser th.tags-column { width:18% }
    ...
    .limits-picker { padding:10px; border:1px solid var(--line); border-radius:8px; background:white } html[data-theme="dark"] .limits-picker { background:#193741 } .limits-picker-head { display:grid; grid-template-columns:minmax(0,1fr) 84px 12px 84px; align-items:center; gap:6px; color:var(--muted); font-size:12px } .limits-picker-head.has-reset { grid-template-columns:minmax(0,1fr) 28px 84px 12px 84px } .limits-picker-name { overflow:hidden; color:var(--ink); font-weight:650; text-overflow:ellipsis; white-space:nowrap } .limits-picker-head input { width:100%; min-height:32px; padding:4px 6px; text-align:right; color:var(--ink); font:12px ui-monospace,monospace } .limits-separator { text-align:center } .limits-reset { display:grid; width:28px; height:28px; padding:0; place-items:center; border:1px solid var(--line); border-radius:50%; color:var(--muted); background:transparent; font:18px/1 system-ui,sans-serif; cursor:pointer } .limits-reset:hover { border-color:var(--accent); color:var(--accent) } .limits-reset:focus-visible { outline:2px solid var(--accent); outline-offset:2px } .limits-slider { position:relative; height:26px; margin-top:7px } .limits-slider-track { position:absolute; inset:10px 7px auto; height:6px; border-radius:999px; background:linear-gradient(to right,var(--line) 0 var(--lower),var(--accent) var(--lower) var(--upper),var(--line) var(--upper) 100%) } .limits-slider input[type=range] { position:absolute; inset:0; z-index:2; width:100%; height:26px; min-height:0; margin:0; padding:0; border:0; pointer-events:none; appearance:none; background:transparent } .limits-slider input[type=range]::-webkit-slider-runnable-track { height:6px; background:transparent } .limits-slider input[type=range]::-webkit-slider-thumb { width:16px; height:16px; margin-top:-5px; border:2px solid white; border-radius:50%; pointer-events:auto; appearance:none; background:var(--accent); box-shadow:0 0 0 1px var(--accent) } .limits-slider input[type=range]::-moz-range-track { height:6px; background:transparent } .limits-slider input[type=range]::-moz-range-thumb { width:14px; height:14px; border:2px solid white; border-radius:50%; pointer-events:auto; background:var(--accent); box-shadow:0 0 0 1px var(--accent) }
...
const request=window.sigvueStatic?.fetch||window.fetch.bind(window);
const api=async path=>{const r=await request(path);if(!r.ok)throw new Error((await r.json()).detail||`Request failed (${r.status})`);return r.json()};
const apiPost=async(path,payload)=>{const r=await request(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!r.ok)throw new Error((await r.json()).detail||`Request failed (${r.status})`);return r.json()};
...
history.pushState=(state,title,path)=>{routeIndex+=1;routeMaximum=routeIndex;sessionStorage.setItem('sigvue-route-maximum',String(routeMaximum));nativePushState({...state,sigvueIndex:routeIndex},title,window.sigvueStatic?window.sigvueStatic.routeUrl(path):path);syncHeaderNavigation()};
function pushRoute(path){history.pushState(null,'',path)}
function followInternalResultLink(event){const link=event.target.closest?.('a[href]');if(!link||event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||link.target==='_blank'||link.hasAttribute('download'))return false;const target=new URL(link.href,location.href),route=window.sigvueStatic?window.sigvueStatic.route(target):target;if(target.origin!==location.origin||!route.pathname.startsWith('/results/'))return false;event.preventDefault();pushRoute(`${route.pathname}${route.search}${route.hash}`);void boot();return true}
...
function plotViewportAtReset(viewport,reset){const entries=Object.entries(viewport||{});if(!entries.length)return true;return entries.every(([name,range])=>{const home=reset?.[`${name}.range`];return Array.isArray(range)&&Array.isArray(home)&&range.length>=2&&home.length>=2&&range.slice(0,2).every((value,index)=>Math.abs(Number(value)-Number(home[index]))<=1e-12)})}
...
function setControlValue(name,value,{commit=false,source='api'}={}){const input=document.querySelector(`[data-control="${CSS.escape(String(name))}"]`);if(!input)return false;const picker=input.closest('[data-limits-picker]');if(picker&&Array.isArray(value)){picker.dispatchEvent(new CustomEvent('sigvue-set',{detail:{value,commit,source}}));return true}input.value=Array.isArray(value)?value.join(','):value;if(input.type==='checkbox')input.checked=String(value).toLowerCase()==='true';input.dispatchEvent(new CustomEvent('sigvue-updated',{bubbles:true,detail:{source}}));if(commit)input.dispatchEvent(new Event('change',{bubbles:true}));return true}
function syncBoundControls(bindings,ranges,commit=true){let changed=false;for(const [axis,target] of Object.entries(bindings||{})){const range=ranges?.[axis];if(!Array.isArray(range)||range.length<2)continue;const edges=range.map(Number).sort((a,b)=>a-b);if(!edges.every(Number.isFinite))continue;if(typeof target==='string')changed=setControlValue(target,edges,{commit,source:'plot'})||changed;else if(Array.isArray(target)&&target.length>=2)for(const [index,name] of target.entries())changed=setControlValue(name,Number(edges[index].toPrecision(12)),{commit:commit&&index===target.length-1,source:'plot'})||changed}return changed}
function bindPlotSelection(plot,view,onViewportChanged){if(plot.dataset.plotSelectionBound)return;plot.dataset.plotSelectionBound='true';plot.on('plotly_selected',event=>{const ranges=selectedPlotRanges(plot,event);if(!Object.keys(ranges).length)return;plotSelections.set(plot.dataset.plotView,ranges);syncBoundControls(view.selection_controls,ranges,true)});plot.on('plotly_relayouting',event=>constrainPlotDuringPan(plot,event));plot.on('plotly_relayout',event=>{if(plot._sigvueUpdating||plot._sigvueResetting)return;if(requestsPlotReset(event)){plotSelections.delete(plot.dataset.plotView);plot._sigvueViewport={};resetPlotAxes(plot,onViewportChanged);return}const viewport=capturePlotViewport(plot,event);if(Object.keys(viewport).length){syncBoundControls(view.viewport_controls,viewport,true);onViewportChanged?.()}});const clear=()=>plotSelections.delete(plot.dataset.plotView);plot.on('plotly_deselect',clear);plot.on('plotly_doubleclick',()=>{clear();plot._sigvueViewport={};resetPlotAxes(plot,onViewportChanged);return false})}
...
function plotlyViewConfig(view){return{...plotlyConfig,modeBarButtonsToAdd:[...new Set([...plotlyConfig.modeBarButtonsToAdd,...(view?.modebar_add||[])])],modeBarButtonsToRemove:[...new Set([...plotlyConfig.modeBarButtonsToRemove,...(view?.modebar_remove||[])])]}}
function managedPlotlyLayout(view){const layout={...(view?.value?.layout||{})};delete layout.uirevision;delete layout.width;delete layout.height;layout.autosize=true;if(view?.drag_mode)layout.dragmode=view.drag_mode;return layout}
...
async function initializePlotlyViews(views,onViewportChanged){const started=performance.now(),jobs=[];document.querySelectorAll('[data-plot-view]').forEach(target=>{const view=views.find(candidate=>candidate.name===target.dataset.plotView);if(view&&view.kind==='plotly')jobs.push(Plotly.newPlot(target,view.value.data||[],managedPlotlyLayout(view),plotlyViewConfig(view)).then(()=>{target._sigvueViewport={};rememberPlotResetRanges(target,view);bindPlotSelection(target,view,(view.rasterized||Object.keys(view.viewport_controls||{}).length)?onViewportChanged:null)}))});await Promise.all(jobs);setPlotlyRuntime(started)}
async function updatePlotlyViews(views){const started=performance.now(),jobs=[];document.querySelectorAll('[data-plot-view]').forEach(target=>{const view=views.find(candidate=>candidate.name===target.dataset.plotView);if(view&&view.kind==='plotly'){const previous=target._sigvueResetRanges,viewport=currentPlotViewport(target),followReset=plotViewportAtReset(viewport,previous),state=plotResetState(view),restored=followReset?{}:restoredPlotViewport(view,viewport,previous,state.reset,state.bounds),layout=layoutWithPlotViewport(view,restored);target._sigvueResetRanges=state.reset;target._sigvueAxisBounds=state.bounds;target._sigvueViewport=Object.fromEntries(Object.entries(restored).filter(([key])=>key.endsWith('.range')).map(([key,range])=>[key.slice(0,-6),range]));target._sigvueUpdating=true;const reaction=Plotly.react(target,view.value.data||[],layout,plotlyViewConfig(view)),applied=followReset?reaction.then(()=>Plotly.relayout(target,state.reset)):reaction;jobs.push(applied.finally(()=>{target._sigvueUpdating=false}))}});await Promise.all(jobs);setPlotlyRuntime(started)}
...
function limitsPickerHtml(control,values){const [lower,upper]=limitsValue(control,values),minimum=Number(control.minimum),maximum=Number(control.maximum),step=Number(control.step)||1,label=control.label||controlLabel(control.name),reset=control.resettable?`<button class="limits-reset" type="button" data-limits-reset aria-label="Reset ${esc(label)}" title="Reset to default">↻</button>`:'';return `<div class="limits-picker" data-limits-picker data-commit-mode="${esc(control.commit_mode||'change')}" data-default-lower="${control.default[0]}" data-default-upper="${control.default[1]}"><div class="limits-picker-head ${control.resettable?'has-reset':''}"><span class="limits-picker-name">${esc(label)}</span>${reset}<input type="number" data-limit-number="lower" value="${lower}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Lower limit"><span class="limits-separator">to</span><input type="number" data-limit-number="upper" value="${upper}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Upper limit"></div><div class="limits-slider"><div class="limits-slider-track"></div><input type="range" data-limit-range="lower" value="${lower}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Lower limit slider"><input type="range" data-limit-range="upper" value="${upper}" min="${minimum}" max="${maximum}" step="${step}" aria-label="Upper limit slider"></div><input type="hidden" data-control="${esc(control.name)}" value="${lower},${upper}"></div>`}
function bindLimitsPickers(onCommit){document.querySelectorAll('[data-limits-picker]').forEach(picker=>{const hidden=picker.querySelector('[data-control]'),lowerNumber=picker.querySelector('[data-limit-number="lower"]'),upperNumber=picker.querySelector('[data-limit-number="upper"]'),lowerRange=picker.querySelector('[data-limit-range="lower"]'),upperRange=picker.querySelector('[data-limit-range="upper"]'),track=picker.querySelector('.limits-slider-track'),minimum=Number(lowerNumber.min),maximum=Number(lowerNumber.max),step=Number(lowerNumber.step)||1,autoCommit=picker.dataset.commitMode!=='explicit';let pointerChanged=null,commitTimer=null;const apply=(lower,upper,commit=false)=>{lower=Math.min(upper-step,Math.max(minimum,Number(lower)));upper=Math.max(lower+step,Math.min(maximum,Number(upper)));lowerNumber.value=lower;upperNumber.value=upper;lowerRange.value=lower;upperRange.value=upper;hidden.value=`${lower},${upper}`;const span=Math.max(Number.EPSILON,maximum-minimum);track.style.setProperty('--lower',`${100*(lower-minimum)/span}%`);track.style.setProperty('--upper',`${100*(upper-minimum)/span}%`);hidden.dispatchEvent(new CustomEvent('sigvue-updated',{bubbles:true}));if(commit&&autoCommit){plotSelections.clear();if(onCommit)void onCommit();else hidden.dispatchEvent(new Event('change',{bubbles:true}))}};const sync=(changed,value,commit)=>{let lower=Number(lowerNumber.value),upper=Number(upperNumber.value),next=Number(value);if(!Number.isFinite(next))next=changed==='lower'?lower:upper;if(changed==='lower')lower=next;else upper=next;apply(lower,upper,commit)};const scheduleCommit=(changed,target)=>{sync(changed,target.value,false);clearTimeout(commitTimer);commitTimer=setTimeout(()=>{sync(changed,target.value,true);commitTimer=null},120)};const beginPointer=(changed,event)=>{pointerChanged=changed;event.currentTarget.setPointerCapture?.(event.pointerId);if(changed==='lower'){lowerRange.style.zIndex='3';upperRange.style.zIndex='2'}else{upperRange.style.zIndex='3';lowerRange.style.zIndex='2'}};const finishPointer=event=>{clearTimeout(commitTimer);commitTimer=null;if(pointerChanged){sync(pointerChanged,event.currentTarget.value,true);pointerChanged=null}if(event.currentTarget.hasPointerCapture?.(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)};lowerNumber.onchange=event=>sync('lower',event.target.value,true);upperNumber.onchange=event=>sync('upper',event.target.value,true);lowerRange.onpointerdown=event=>beginPointer('lower',event);upperRange.onpointerdown=event=>beginPointer('upper',event);lowerRange.oninput=event=>scheduleCommit('lower',event.currentTarget);upperRange.oninput=event=>scheduleCommit('upper',event.currentTarget);lowerRange.onchange=event=>{if(pointerChanged!=='lower')sync('lower',event.target.value,true)};upperRange.onchange=event=>{if(pointerChanged!=='upper')sync('upper',event.target.value,true)};lowerRange.onpointerup=finishPointer;upperRange.onpointerup=finishPointer;lowerRange.onpointercancel=finishPointer;upperRange.onpointercancel=finishPointer;lowerNumber.onkeydown=upperNumber.onkeydown=event=>{if(event.key==='Enter')event.currentTarget.blur()};picker.addEventListener('sigvue-set',event=>{const value=event.detail?.value;if(Array.isArray(value)&&value.length>=2)apply(value[0],value[1],Boolean(event.detail?.commit))});picker.querySelector('[data-limits-reset]')?.addEventListener('click',()=>apply(picker.dataset.defaultLower,picker.dataset.defaultUpper,true));apply(lowerNumber.value,upperNumber.value,false)})}
...
function batchBrowseChoices(batch){const choices=[];if(batch?.result_browser_url)choices.push({label:'All batch results',url:batch.result_browser_url,scope:'all'});for(const action of batch?.actions||[]){const url=action.collection_browser_url||action.result_browser_url;if(url)choices.push({label:action.label,url,scope:action.value})}return choices}
function batchFolderHtml(batch){const choices=batchBrowseChoices(batch),action=batchBrowseAction(batch),content=`${batchFolderIcon}<span class="batch-folder-label">Browse</span>`;if(choices.length){return `<details class="batch-browse-menu" data-batch-browse-menu><summary class="batch-folder" data-batch-folder data-batch-folder-scope="collection" aria-label="Browse batch results" title="Browse batch results">${content}<span class="batch-browse-caret" aria-hidden="true">▼</span></summary><div class="batch-browse-popover">${choices.map(choice=>`<a class="batch-browse-choice" data-batch-browse-action="${esc(choice.scope)}" href="${esc(choice.url)}">${batchFolderIcon}<span>${esc(choice.label)}</span></a>`).join('')}</div></details>`}const url=action?.result_browser_url,label=action?`Browse results for ${action.label}`:'No batch results yet';return url?`<a class="batch-folder" data-batch-folder href="${esc(url)}" aria-label="${esc(label)}" title="${esc(label)}">${content}</a>`:`<span class="batch-folder" data-batch-folder aria-disabled="true" aria-label="${esc(label)}" title="${esc(label)}">${content}</span>`}
...
function closeBatchMenus(except=null){document.querySelectorAll('[data-batch-menu][open],[data-batch-browse-menu][open]').forEach(menu=>{if(menu!==except)menu.open=false})}
...
async function openItem(wid,wname,iid,navigate=true,controlValues={},preservePlayback=false){
  ...
  try{const request=async values=>api(`/workspaces/${encodeURIComponent(wid)}/items/${encodeURIComponent(iid)}?${new URLSearchParams(values)}`),windowValues=()=>windowEnd==null?{}:{__window_start_seconds:windowStart,__window_end_seconds:windowEnd},segmentValues=()=>segmentId==null?{}:{__segment_id:segmentId};let data=await request({...controlValues,...windowValues(),...segmentValues(),__theme:resolvedTheme(),__playback_time_seconds:playbackPosition});let p=data.page,requestGeneration=0;const isPlayback=['seek','live'].includes(p.playback.mode),isWindowed=p.playback.mode==='windowed',isSegmented=p.playback.mode==='segmented';annotations=p.annotation?.entries||[];
    ...
    document.querySelector('#download-form').onsubmit=async event=>{event.preventDefault();const button=event.currentTarget.querySelector('button'),scope=document.querySelector('#export-scope').value,format=document.querySelector('#export-format').value;button.disabled=true;button.textContent='Preparing…';try{const job=await apiPost(`/workspaces/${encodeURIComponent(wid)}/items/${encodeURIComponent(iid)}/exports`,{control_values:{...selected(),__playback_time_seconds:playbackPosition},scope,format});let status;do{await new Promise(resolve=>setTimeout(resolve,350));status=await api(job.status_url)}while(status.status==='pending'||status.status==='running');if(status.status==='error')throw new Error(status.detail);for(const file of status.files){if(window.sigvueStatic){await window.sigvueStatic.download(file)}else{const link=document.createElement('a');link.href=file.url;link.download=file.name;link.click()}}headerDownload.open=false}catch(error){alert(`Export failed: ${error.message}`)}finally{button.disabled=false;button.textContent='Download'}};
    const refresh=async(includeStatic=false,commitRequestedTheme=false,rasterOnly=false,extraValues={})=>{const generation=++requestGeneration,result=await request({...selected(),...extraValues,__playback_time_seconds:playbackPosition,__include_static_views:includeStatic});if(generation!==requestGeneration)return false;const browserStarted=performance.now();data=result;Object.assign(playbackConfig,result.page.playback);p=result.page;p.playback=playbackConfig;annotationTimelineColorControl=p.annotation?.timeline_color_control||null;if(Array.isArray(p.annotation?.entries))annotations=p.annotation.entries;if(playbackFollowLive)playbackPosition=playbackConfig.duration_seconds;if(commitRequestedTheme)await preloadMatplotlibViews(p.rendered_views);const render=async()=>{if(commitRequestedTheme)applyTheme();if(!rasterOnly)updateStatistics(p.statistics,p.runtime_statistics);const mounted=mountRenderedViews(p.rendered_views),mountedNames=new Set(mounted.map(view=>view.name)),updates=p.rendered_views.filter(view=>!mountedNames.has(view.name));await initializePlotlyViews(mounted,scheduleRasterRefresh);await updatePlotlyViews(updates);if(!rasterOnly){await updateMatplotlibViews(p.rendered_views);updateGenericViews(p.rendered_views)}setClientRuntime('browser-runtime',performance.now()-browserStarted)};if(commitRequestedTheme)await commitTheme(render);else await render();return true};activeViewChanged=async()=>{if(!p.lazy_views)return true;if(isPlayback)clearInterval(playbackTimer);const applied=await refresh(true);if(isPlayback&&applied)startFrameworkPlayback(p.playback,refresh);else if(isWindowed&&applied)startFrameworkWindowed(p.playback,refresh,p.controls);else if(isSegmented&&applied)startFrameworkSegmented(p.playback,refresh);return applied};activeThemeRefresh=async()=>{if(isPlayback)clearInterval(playbackTimer);const applied=await refresh(true,true);if(isPlayback&&applied)startFrameworkPlayback(p.playback,refresh);else if(isWindowed&&applied)startFrameworkWindowed(p.playback,refresh,p.controls);else if(isSegmented&&applied)startFrameworkSegmented(p.playback,refresh)};
    ...
    bindLimitsPickers(settingsChanged);document.querySelectorAll('[data-action]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await refresh(true,false,false,{__action:button.dataset.action})}finally{button.disabled=false}});if(isPlayback)startFrameworkPlayback(p.playback,refresh);else if(isWindowed)startFrameworkWindowed(p.playback,refresh,p.controls);else if(isSegmented)startFrameworkSegmented(p.playback,refresh);else if(p.refresh.enabled)startFrameworkRefresh(p.refresh,refresh);document.querySelectorAll('[data-control]').forEach(x=>{if(x.closest('[data-limits-picker]'))return;x.onchange=()=>{plotSelections.clear();return settingsChanged()};if(x.type==='color')x.oninput=()=>{const swatch=x.closest('[data-style-picker]')?.querySelector('[data-style-swatch]');if(swatch)swatch.style.background=x.value;updateAnnotationMarkerColor()}});document.querySelector('#home')?.addEventListener('click',()=>catalog());document.querySelector('#back').onclick=()=>items(wid,wname,true,data.item.navigation_path||[])
  }catch(e){fail(e)}}
async function boot(reload=false){const route=window.sigvueStatic?window.sigvueStatic.route():location,parts=route.pathname.split('/').filter(Boolean).map(decodeURIComponent),workspaceUrl=reload?'/workspaces?reload=1':'/workspaces';if(parts[0]==='results'&&['job','saved'].includes(parts[1])&&parts[2]&&!parts[3]){const path=new URLSearchParams(route.search).get('path')||'',directory=path.split('/').filter(Boolean);return resultBrowser(parts[1],parts[2],null,false,directory)}if(parts[0]==='results'&&parts[1]&&parts[2]&&parts[3])return resultBrowser(parts[1],parts[2],parts[3],false,parts.slice(4));if(parts[0]!=='workspace'){if(reload)await api(workspaceUrl);return catalog(false)}try{const {workspaces}=await api(workspaceUrl);singleWorkspaceMode=workspaces.length===1;const workspace=workspaces.find(w=>w.id===parts[1]);if(!workspace)return catalog(false);if(parts[2]==='item'&&parts[3])return openItem(workspace.id,workspace.name,parts[3],false);if(parts[2]==='browse')return items(workspace.id,workspace.name,false,parts.slice(3));return items(workspace.id,workspace.name,false)}catch(e){fail(e)}}
...
class SigvueApp:
        ...
        with self._batch_lock:
            for choice in choices:
                ...
                collection_browser_url = (
                    self._batch_destination_collection_url(
                        destinations[choice.value],
                        workspace_id,
                        item_id,
                        choice.value,
    ...
    def _batch_destination_collection_url(
        self,
    ...
    def _declared_batch_status(
        ...
        if all((directory / name).exists() for name in destination.files):
            return {
                ...
                "result_browser_url": self._batch_destination_collection_url(
                    destination,
                    workspace_id,
                    item_id,
                    action,
            ...
            rendered_views.append(
                {
                    ...
                    "viewport_controls": view.viewport_controls,
                    "selection_controls": view.selection_controls,
                    "drag_mode": view.drag_mode,
                    "modebar_add": view.modebar_add,
                    "modebar_remove": view.modebar_remove,
...
def _make_handler(app: SigvueApp) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def handle(self) -> None:
            """Ignore clients that leave while an in-flight response is written."""
            try:
                super().handle()
