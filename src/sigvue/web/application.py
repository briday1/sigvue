...
function bindPlotSelection(plot,onViewportChanged){if(plot.dataset.plotSelectionBound)return;plot.dataset.plotSelectionBound='true';plot.on('plotly_selected',event=>{const ranges=selectedPlotRanges(plot,event);if(Object.keys(ranges).length)plotSelections.set(plot.dataset.plotView,ranges)});plot.on('plotly_relayouting',event=>constrainPlotDuringPan(plot,event));plot.on('plotly_relayout',()=>{if(!plot._sigvueUpdating)onViewportChanged?.()});const clear=()=>plotSelections.delete(plot.dataset.plotView);plot.on('plotly_deselect',clear);plot.on('plotly_doubleclick',()=>{clear();resetPlotAxes(plot);return false})}
