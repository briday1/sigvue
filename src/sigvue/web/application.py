...
function restoredPlotViewport(plot,viewport,previousReset){const update={};for(const [name,range] of Object.entries(viewport||{})){if(!plot?._fullLayout?.[name]||!Array.isArray(range))continue;const previous=previousReset?.[`${name}.range`],current=plot._sigvueResetRanges?.[`${name}.range`],translated=translatedAxisRange(range,previous,current),bounds=plot._sigvueAxisBounds?.[name],restored=bounds?clampedAxisRange(translated,bounds):translated;update[`${name}.range`]=restored;update[`${name}.autorange`]=false}return update}
...
function managedPlotlyLayout(view){const layout={...(view?.value?.layout||{})};delete layout.uirevision;return layout}
...
async function updatePlotlyViews(views){const started=performance.now(),jobs=[];document.querySelectorAll('[data-plot-view]').forEach(target=>{const view=views.find(candidate=>candidate.name===target.dataset.plotView);if(view&&view.kind==='plotly'){const previous=target._sigvueResetRanges,viewport=currentPlotViewport(target);target._sigvueUpdating=true;jobs.push(Plotly.react(target,view.value.data||[],managedPlotlyLayout(view),plotlyConfig).then(()=>{rememberPlotResetRanges(target,view);const changed=changedPlotResetRanges(previous,target._sigvueResetRanges),restored=restoredPlotViewport(target,viewport,previous),images=view.rasterized?{'images':view.value.layout?.images||[]}:{},update={...changed,...restored,...images};target._sigvueViewport=Object.fromEntries(Object.entries(restored).filter(([key])=>key.endsWith('.range')).map(([key,range])=>[key.slice(0,-6),range]));return Object.keys(update).length?Plotly.relayout(target,update):undefined}).finally(()=>{target._sigvueUpdating=false}))}});await Promise.all(jobs);setPlotlyRuntime(started)}
