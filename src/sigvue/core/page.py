...
class PlaybackConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode == "windowed":
            ...
            if any(not row for row in self.overview_heatmap):
                raise ValueError("Windowed overview heatmap rows cannot be empty")
            if (
                self.overview_heatmap
                and len({len(row) for row in self.overview_heatmap}) != 1
            ):
                raise ValueError("Windowed overview heatmap must be rectangular")
            if not all(
            ...
            if self.overview_switcher_key and not self.overview_series:
                raise ValueError(
                    "A windowed overview switcher requires overview series"
                )
            ...
            if self.overview_durations_seconds:
                if len(self.overview_durations_seconds) != len(self.overview_series):
                    raise ValueError(
                        "Windowed overview durations must match the overview series"
                    )
                if not all(
            ...
            if not 0 < self.minimum_window_seconds <= self.duration_seconds:
                raise ValueError(
                    "Windowed minimum size must be positive and within the duration"
                )
        if self.mode == "segmented":
            ...
            if any(
                segment.start_seconds + segment.duration_seconds > self.duration_seconds
...
class PageDefinition:
    ...
    def validate(self) -> None:
        invalid_placements = {control.placement for control in self.controls} - {
            "details",
            "inline",
        }
        if invalid_placements:
            raise ValueError(
                f"Unknown control placements: {', '.join(sorted(invalid_placements))}"
            )
        invalid_updates = {view.update_policy for view in self.views} - {
            "static",
            "dynamic",
        }
        if invalid_updates:
            raise ValueError(
                f"Unknown view update policies: {', '.join(sorted(invalid_updates))}"
            )
        invalid_axis_navigation = {view.axis_navigation for view in self.views} - {
            "free",
            "bounded",
        }
