...
from examples.readers import sigmf_reader
from examples.style import add_time_frequency_annotation_regions
from sigvue import Annotation, AnnotationRequest, ExportRequest


class SigMFSupportTests(unittest.TestCase):
    @staticmethod
    ...
    def test_generator_module_entry_point_imports_local_format_support(self):
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "examples.scripts.generate_all",
                "--help",
            ],
    ...
    def test_reader_uses_logical_offsets_and_active_capture_metadata(self):
        with TemporaryDirectory() as directory:
            ...
            resources = sigmf_reader(root).resources()
            self.assertEqual(
    ...
    def test_window_annotation_and_export_edges_are_safe(self):
        with TemporaryDirectory() as directory:
            ...
            annotator = SigMFAnnotator(generator="Local capability test")
