...
def plot_constellation(products: CommsProducts) -> go.Figure:
    return constellation_figure(products)


def plot_eye(products: CommsProducts) -> go.Figure:
    return eye_figure(products)


def plot(products: CommsProducts, *, view: str = "constellation") -> go.Figure:
    builders = {
        "constellation": plot_constellation,
        "eye": plot_eye,
    }
    try:
        return builders[view](products)
    except KeyError as error:
        raise ValueError(
            f"Unknown communications view {view!r}; choose from {tuple(builders)}"
        ) from error
