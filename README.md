...
```mermaid
flowchart LR
    Native["Paths, URLs, keys, or collections"] --> Reader
    Reader -->|exact selected data| Workspace
    Workspace -->|"view(data, ui)"| Browser["Tabs, plots, tables, controls, and actions"]

    Reader -. "discover / open / load" .-> Script["Notebook, test, or batch script"]
    Workspace -. "same analysis and plot functions" .-> Script
```
...
sequenceDiagram
    ...
    participant Reader
    participant View as view(data, ui)
    participant Lazy as visible lazy callback

    Browser->>Workspace: open item with UI state
    Workspace->>Reader: discover and open native reference
    Workspace->>Reader: read exact requested buffer
    Reader-->>Workspace: selected domain data
    Workspace->>View: selected data and UI
    View->>View: optional direct or cached processing
    View-->>Workspace: declared layout and lazy callbacks
    Browser->>Lazy: request visible plot/table/text
    Lazy-->>Browser: rendered value
    Note over Lazy: hidden callbacks are not executed
```
...
flowchart TB
    Workspace --> Reader
    Workspace --> View["view(data, ui)"]
    Workspace -. optional .-> Annotator
    Workspace -. optional .-> Exporter
    Workspace -. optional .-> Batch

    Reader --> Discovery["native discovery and exact reads"]
    View --> Science["author-defined processing, plotting, and layout"]
    Annotator --> Review["read and persist domain annotations"]
    Exporter --> Download["serialize author-selected results"]
    Batch --> Actions["repeatable item or workspace jobs"]
```
